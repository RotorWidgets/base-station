...
class EventModelMixin(TimeStampedModel, TitleSlugDescriptionModel):
    ...
    def __str__(self):
        return "{title} ({start})".format(
            title=self.title, start=arrow.get(self.start).format('MMM D, YYYY'))
...
class Occurrence(EventModelMixin):
    ...
    def set_template(self, field):
        ...
        event_template = getattr(self.event, '{}_template'.format(field))

        try:
            rendered = render_template(event_template)
        except TemplateError:
            default_template_string = getattr(self, 'default_{}_template'.format(field))
            rendered = render_template(default_template_string)
            logger.warning((
                '{occurrence!r} has errors in it\'s event\'s {field}_template'.format(
                    occurrence=self, field=field)))
